...
parser.add_argument("--root", default='/HOME/paratera_xy/pxy368/HDD_POOL/CODE/scripts/math_dataset', type=str)
parser.add_argument("--model_path", default='/HOME/paratera_xy/pxy368/HDD_POOL/CODE/models/QVQ-72B-Preview', type=str)
parser.add_argument("--model", default='qwen2vl-7b', type=str)
parser.add_argument("--out_dir", default='/HOME/paratera_xy/pxy368/HDD_POOL/yuting/inference/model_answer', type=str)
parser.add_argument("--system_prompt", default="Please reason step by step, and put your final answer within \\boxed{}.", type=str)
args = parser.parse_args()

MODEL_PATH = args.model_path
if args.benchmark == 'wemath':
    BENCHMARK_PATH = os.path.join(args.root, 'We-Math/we-math-2.json')
if args.benchmark == 'mathvista':
    BENCHMARK_PATH = os.path.join(args.root, 'MathVista/mathvista-new.json')
if args.benchmark == 'mathverse':
    BENCHMARK_PATH = os.path.join(args.root, 'MathVerse/mathverse-2.json')
if args.benchmark == 'mathvision':
    BENCHMARK_PATH = os.path.join(args.root, 'MathVision/mathvision-1.json')
if args.benchmark == 'chartqa':
    BENCHMARK_PATH = os.path.join(args.root, 'ChartQA/chartqa_test-1.json')
if args.benchmark == 'clevr':
    BENCHMARK_PATH = os.path.join(args.root, 'clevr-math/clevr-test-1.json')
if args.benchmark == 'scienceqa':
    BENCHMARK_PATH = os.path.join(args.root, 'ScienceQA/scienceqa_test-1.json')
if args.benchmark == 'm3cot':
    BENCHMARK_PATH = os.path.join(args.root, 'M3CoT/m3cot-test-1.json')
...
outputs = []
correct_cnt = 0
for index in range(int(len(total_data)/1000)+1):
    if 1000*(index+1)<len(total_data):
        inputs_data = total_data[1000*index:1000*(index+1)]
    else:
        inputs_data = total_data[1000*index:]
    prompt_list = []
